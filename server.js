...
<style>body{font-family:sans-serif;background:#0a0c12;color:#edf2f9;display:flex;flex-direction:column;align-items:center;justify-content:center;min-height:100vh;margin:0;gap:16px;}
button{background:#25d366;color:#000;border:none;padding:14px 28px;border-radius:12px;font-size:15px;font-weight:700;cursor:pointer;}</style>
</head>
<body><div style="font-size:32px">💬</div><h2>Connect WhatsApp Number</h2>
<button onclick="launchSignup()">Connect via Meta</button>
<div id="status"></div>
<script>
  window.fbAsyncInit=function(){ FB.init({appId:'${META_APP_ID}',cookie:true,xfbml:true,version:'${META_API_VERSION}'}) };
  ...
  function launchSignup() {
    ...
    FB.login(function(response){
      ...
      }else{ setStatus('Cancelled'); document.querySelector('button').disabled=false }
...
app.post('/api/wa/connect', async (req, res) => {
  ...
  try {
    const tokenRes = await fetch(`https://graph.facebook.com/${META_API_VERSION}/oauth/access_token?client_id=${META_APP_ID}&client_secret=${META_APP_SECRET}&code=${encodeURIComponent(code)}`)
    const tokenData = await tokenRes.json()
    if (tokenData.error) return res.status(400).json({ error: tokenData.error.message })
    
    const accessToken = tokenData.access_token
    const wabaRes = await fetch(`https://graph.facebook.com/${META_API_VERSION}/me/whatsapp_business_accounts`, { headers: { 'Authorization': `Bearer ${accessToken}` } })
    const wabaData = await wabaRes.json()
    const wabaId = wabaData.data?.[0]?.id
    if (!wabaId) return res.status(400).json({ error: 'No WABA found' })
    
    const phoneRes = await fetch(`https://graph.facebook.com/${META_API_VERSION}/${wabaId}/phone_numbers?fields=id,display_phone_number,verified_name,quality_rating`, { headers: { 'Authorization': `Bearer ${accessToken}` } })
    const phoneData = await phoneRes.json()
    const phone = phoneData.data?.[0]
    if (!phone) return res.status(400).json({ error: 'No phone numbers found' })
    
    await fetch(`https://graph.facebook.com/${META_API_VERSION}/${wabaId}/subscribed_apps`, { method: 'POST', headers: { 'Authorization': `Bearer ${accessToken}` } })
    
    const insertRes = await sbFetch('/wa_accounts', {
      method: 'POST',
      body: JSON.stringify({
        user_id, waba_id: wabaId, phone_number_id: phone.id,
        phone_number: phone.display_phone_number, display_name: phone.verified_name,
        access_token: accessToken, quality_rating: phone.quality_rating || 'GREEN',
        is_active: true, messages_sent_today: 0,
        last_reset_date: new Date().toISOString().split('T')[0],
        created_at: new Date().toISOString(), updated_at: new Date().toISOString()
      })
    })
    if (!insertRes.ok) return res.status(500).json({ error: 'Failed to save' })
...
// ================================================================
// QUEUE PROCESSOR (called by cron every 2 seconds)
// ================================================================
app.post('/api/queue/process', async (req, res) => {
  const { campaign_id } = req.body
  const result = await callEdge('campaignProcessQueue', { campaign_id })
  res.json(result)
})
    ...
    for (const change of (entry.changes || [])) {
      if (change.field === 'messages') {
        for (const status of (change.value.statuses || [])) {
          await callEdge('internalDeliveryWebhook', { id: status.id, status: status.status, errors: status.errors || [] })
        }
...
const server = app.listen(PORT, () => {
  ...
  // Start queue processor (runs every 2 seconds)
  setInterval(async () => {
    try {
  ...
  // Self-pinger
  setInterval(async () => {
    try { await fetch(`${SELF_URL}/health`) } catch (err) {}
  }, 14 * 60 * 1000)
})
